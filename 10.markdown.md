...
graph TD
subgraph "最终结果"
    J["前六位不在交集列表的7位wifi geohash(wifi_only_geohash)"]
    K["前六位不在交集列表的7位essid geohash(essid_only_geohash)"]
    L["前六位在交集列表的7位geohash(both_geohash)"]
    H[最大时间戳]
end

subgraph "聚合中间数据"
    F[essid7位去重geohash列表]
    G[wifi7位去重geohash列表]
    I[6位geohash的交集列表]
end

A["上一个分区数据(T-2)"]-->|拆分both_geohash到wifi_only/essid_only|B[转换成和增量数据相同格式]
C["type27、type11增量数据(T-1)"]-->D[合并结果]
B-->D
D-->|字段选择+分组|E[聚合后结果]
...
F-->|prefix intersection|I
G-->|prefix intersection|I
