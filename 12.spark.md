...
- [12. Spark](#12-spark)
  ...
  - [性能调优与优化](#性能调优与优化)
    ...
    - [常见性能问题](#常见性能问题)
    - [监控与诊断](#监控与诊断)
      - [Spark Web UI - SQL 页面解读](#spark-web-ui---sql-页面解读)
      - [Details 中 Physical Plan 详解](#details-中-physical-plan-详解)
...
df = spark.createDataFrame([(1, "  Alice "), (2, "Bob  ")], ["id", "name"]) \
       .select(F.expr("normalize(name) as norm"))
df.show()

# 2) Pandas UDF（更高性能，Arrow 加速）
@F.pandas_udf(T.LongType())
def strlen_sum_udf(s: 'pandas.Series') -> 'pandas.Series':
    return (s.fillna('').str.len()).astype('int64')

spark.createDataFrame([(1, "a"), (2, "bbb"), (3, None)], ["id", "name"]) \
    .select(strlen_sum_udf(F.col("name")).alias("len")) \
    .show()
```
...
| 配置参数 | 含义 | 说明 |
| --- | --- | --- |
| `spark.executor.memoryOverhead=1024` | Executor 额外内存（非 JVM 堆） | 用于存放直接内存、非堆数据（如 shuffle buffer、off-heap），默认值较小，1GB 可以避免大数据量 shuffle 时 OOM。 |
| `spark.executor.extraJavaOptions="-XX:+UseG1GC -XX:+UseStringDeduplication -XX:InitiatingHeapOccupancyPercent=35 -XX:MaxGCPauseMillis=200"` | JVM GC 参数 | 使用 G1GC，开启字符串去重，降低内存占用；设置 G1 触发 GC 的堆占用比例 35%，目标最大 GC 停顿 200ms。 |
| `spark.executor.heartbeatInterval=60s` | Executor 心跳间隔 | 心跳用于汇报 Executor 状态，过长可能导致 driver 误判 executor 失联。 |
| `spark.sql.adaptive.enabled=true` | 启用 AQE | Adaptive Query Execution，可在运行时根据数据统计动态调整执行计划。 |
| `spark.sql.adaptive.skewJoin.enabled=true` | 启用倾斜 Join 优化 | AQE 自动检测并拆分数据倾斜的分区。 |
| `spark.sql.adaptive.skewJoin.skewedPartitionThresholdInBytes=1g` | 倾斜分区大小阈值 | 当分区数据量超过 1GB 时认为可能倾斜，会被拆分。 |
| `spark.sql.adaptive.skewJoin.skewedPartitionFactor=5` | 倾斜判断倍数 | 某个分区大小超过 **中位数分区大小的 5 倍** 时，判定为倾斜。 |
| `spark.sql.parquet.binaryAsString=true` | 将 Parquet Binary 当 String | 读取时将 Binary 类型字段按 UTF-8 字符串解析。 |
| `spark.sql.parquet.compression.codec=snappy` | Parquet 压缩方式 | snappy 压缩速度快、解压迅速，适合中间数据存储。 |
| `spark.serializer=org.apache.spark.serializer.KryoSerializer` | 使用 Kryo 序列化 | 比默认 Java 序列化更快、压缩率更高。 |
| `spark.kryoserializer.buffer.max=128m` | Kryo 缓冲区最大值 | 防止序列化大对象时缓冲区不足。 |
| `spark.kryoserializer.buffer=128k` | Kryo 初始缓冲区 | 初始化时的序列化缓冲区大小，小对象时可节省内存。 |
| `spark.shuffle.consolidateFiles=true` | Shuffle 文件合并 | 减少 Shuffle 文件数量，降低文件系统压力。 |
| `spark.shuffle.io.preferDirectBufs=true` | 使用 DirectBuffer | 在 Shuffle 时直接使用 off-heap 内存，减少 GC 压力。 |
| `spark.shuffle.io.maxRetries=10` | Shuffle 失败重试次数 | 网络或磁盘临时异常时的最大重试次数。 |
| `spark.shuffle.io.retryWait=20s` | Shuffle 重试等待时间 | 避免频繁重试导致集群压力。 |
| `spark.shuffle.compress=true` | Shuffle 数据压缩 | 传输前压缩 Shuffle 数据，减少网络 IO。 |
| `spark.shuffle.spill.compress=true` | 溢写数据压缩 | 溢写到磁盘的中间数据压缩，减少磁盘 IO。 |
| `spark.shuffle.file.buffer=4m` | Shuffle 写缓冲大小 | 写磁盘时的 buffer 大小，适当调大可减少 flush 次数。 |
| `spark.network.timeout=600s` | 网络超时 | 心跳、Shuffle、RPC 等的最大超时，长任务可避免超时误判。 |
| `spark.reducer.maxSizeInFlight=96m` | 每个 Reducer 拉取数据的批次大小 | 较大可减少请求次数，但会增加内存占用，过大会导致内存压力。 |
