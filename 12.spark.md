...
| 方法 | 存储空间 | 性能 | 唯一性保证 | 重复概率 | 可读性 | 排序性 | 适用场景 |
|------|----------|------|------------|----------|--------|--------|----------|
| **monotonically_increasing_id()** | 8字节(Long) | 极高 | 全局唯一 | 0% | 差 | 是 | 行标识、排序需求 |
| **uuid()** | 36字节(String) | 高 | 极高 | ~10^-37 | 中 | 否 | 分布式环境、外部系统对接 |
| **hash()** | 8字节(Long) | 极高 | 数据相同则相同 | 1/2^64 ≈ 5.4×10^-20 | 差 | 否 | 数据去重、快速查找 |
| **xxhash64()** | 8字节(Long) | 极高 | 数据相同则相同 | 1/2^64 ≈ 5.4×10^-20 | 差 | 否 | 高性能哈希、分桶 |
| **md5()** | 32字节(String) | 中 | 极高 | 1/2^128 ≈ 2.9×10^-39 | 差 | 否 | 数据完整性校验、去重 |
| **sha1()** | 40字节(String) | 中 | 极高 | 1/2^160 ≈ 6.8×10^-49 | 差 | 否 | 安全性要求较高场景 |
| **sha2(256)** | 64字节(String) | 低 | 极高 | 1/2^256 ≈ 8.6×10^-78 | 差 | 否 | 高安全性要求 |
| **timestamp_id** | 可变 | 高 | 高 | 取决于时间精度 | 中 | 是 | 时间相关业务、调试 |
| **custom_id** | 可变 | 中 | 依赖设计 | 取决于设计方案 | 高 | 是 | 业务友好、人工可读 |
