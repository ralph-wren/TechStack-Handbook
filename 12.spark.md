...
- **不可变性**：数据一旦创建不可修改，保证了数据的一致性
- **分布式**：数据分布在集群的多个节点上
- **弹性容错**：通过血统信息自动容错恢复
- **惰性求值**：Transformation操作延迟执行，直到Action操作触发
- **分区计算**：支持并行计算，提高处理效率
...
| **适用场景** | 底层操作 | SQL查询 | 类型安全要求高 |

### 架构原理题

**3. Spark的核心组件有哪些？各自的作用是什么？**

- **Driver Program**：运行应用程序main函数，创建SparkContext，协调任务执行
- **SparkContext**：Spark应用程序的入口点，负责与集群建立连接
- **Cluster Manager**：集群资源管理器（YARN、Mesos、Kubernetes等）
- **Executor**：运行在Worker节点上的JVM进程，执行具体的Task
- **DAGScheduler**：将作业分解为Stage，按依赖关系调度
- **TaskScheduler**：将Task分发到Executor执行

**4. Spark的任务调度流程是怎样的？**

1. **用户提交Action操作**
2. **SparkContext创建作业**，调用DAGScheduler.runJob
3. **DAGScheduler分析依赖关系**，划分Stage
4. **按拓扑顺序提交Stage**，创建TaskSet
5. **TaskScheduler接收TaskSet**，分配资源
6. **Task分发到Executor**执行
7. **收集执行结果**，完成作业
...
- **加盐处理**：为倾斜的key添加随机前缀，分散到多个分区
- **两阶段聚合**：先本地聚合，再全局聚合
- **自定义分区器**：根据业务逻辑自定义分区策略
- **广播Join**：将小表广播，避免Shuffle
- **增加分区数**：提高并行度，减少单个分区数据量

**6. Spark性能调优的主要策略有哪些？**

- **资源配置调优**：合理设置Executor内存、核心数、数量
- **代码优化**：使用合适的算子、避免重复计算、使用广播变量
- **缓存策略**：合理选择存储级别，及时释放缓存
- **Shuffle优化**：减少Shuffle操作，调整分区数
- **序列化优化**：使用Kryo序列化器
- **垃圾回收优化**：调整GC参数
...
- **Spark UI监控**：查看Jobs、Stages、Storage、Executors页面
- **应用程序日志**：分析Driver和Executor日志
- **系统指标监控**：CPU、内存、磁盘、网络使用率
- **第三方工具**：Ganglia、Nagios、Prometheus等
- **自定义监控**：通过Spark Listener收集指标

**8. Spark Streaming与Structured Streaming的区别？**

| 特性 | Spark Streaming | Structured Streaming |
|------|-----------------|---------------------|
| **处理模型** | 微批次DStream | 连续流表 |
...
**9. Shuffle的实现原理是什么？不同Shuffle类型的区别？**

**Shuffle实现原理**：
- **Map端**：将数据按分区ID写入本地磁盘文件
- **Reduce端**：从各个Map端拉取对应分区的数据
- **协调机制**：通过MapOutputTracker管理元数据

**Shuffle类型对比**：
- **Hash Shuffle**：每个Map Task为每个Reduce Task创建文件，文件数M×N
- **Sort Shuffle**：每个Map Task创建一个文件，按分区ID排序
- **Tungsten Sort**：使用堆外内存，优化排序性能

**10. Spark的内存管理机制是怎样的？**

Spark使用统一内存管理（Unified Memory Manager）：
- **存储内存**：用于RDD缓存、广播变量
- **执行内存**：用于Shuffle、Join、聚合等操作
- **动态调整**：存储内存和执行内存可以相互借用
- **内存分区**：堆内内存和堆外内存分别管理