...
```mermaid
graph TD
    A[Spark应用提交] --> B[YarnClient模式选择]
    B --> C{部署模式}
    C -->|client模式| D[Driver在客户端运行]
    C -->|cluster模式| E[Driver在YARN容器中运行]
    D --> F[启动ApplicationMaster]
    E --> G[ApplicationMaster包含Driver]
    F --> H[AM向RM申请资源]
    G --> H
    H --> I[RM分配Container]
    I --> J[启动NodeManager上的Executor]
    J --> K[Executor注册到Driver]
    K --> L[开始任务执行]
    L --> M[任务完成，释放资源]
    
    F --> F1[上传应用JAR到HDFS]
    F --> F2[创建应用提交上下文]
    F --> F3[提交应用到ResourceManager]
    
    J --> J1[下载应用JAR]
    J --> J2[启动ExecutorBackend]
    J --> J3[初始化Executor]
...
private[spark] class ApplicationMaster(
  ...
  private class NMCallbackHandler extends NMClientAsync.CallbackHandler {
    override def onContainerStarted(containerId: ContainerId, allServiceResponse: ju.Map[String, ByteBuffer]): Unit = {
    ...
    override def onGetContainerStatusError(containerId: ContainerId, t: Throwable): Unit = {
      logWarning(s"Failed to get status for container $containerId", t)
    }
...
```mermaid
graph TD
    A[Spark应用提交] --> B[KubernetesClusterManager]
    B --> C[创建Driver Pod]
    C --> D[Driver Pod启动]
    D --> E[KubernetesExecutorBackend]
    E --> F[请求Executor Pods]
...
private[spark] class KubernetesClusterManager extends ExternalClusterManager with Logging {
  ...
  override def createSchedulerBackend(
    ...
    val kubernetesConf = KubernetesUtils.parseMasterUrl(masterURL)
...
private[spark] class KubernetesClusterSchedulerBackend(
    ...
    sc: SparkContext,
    kubernetesConf: KubernetesConf) extends CoarseGrainedSchedulerBackend(scheduler, sc.env.rpcEnv) {
  
  // Kubernetes客户端
  private val kubernetesClient = SparkKubernetesClientFactory.createKubernetesClient(kubernetesConf)
  private val namespace = kubernetesConf.namespace
  
  // Executor管理
  private val executorPodManager = new ExecutorPodManager(kubernetesClient, kubernetesConf)
  private val requestExecutorsService = ThreadUtils.newDaemonSingleThreadScheduledExecutor("k8s-request-executors")
  
  // 动态资源分配
  private val podAllocationDelay = kubernetesConf.get("spark.kubernetes.allocation.batch.delay", "1s")
  private val podAllocationSize = kubernetesConf.get("spark.kubernetes.allocation.batch.size", "5").toInt
  
  @volatile private var totalRegisteredExecutors = 0
  @volatile private var totalExpectedExecutors = 0
  
  override def start(): Unit = {
    super.start()
    
    logInfo("Starting Kubernetes cluster scheduler backend")
    
    // 启动Executor监控
    executorPodManager.start()
    
    // 启动资源请求调度器
    requestExecutorsService.scheduleWithFixedDelay(
      new Runnable {
        override def run(): Unit = {
      ...
      0,
      Utils.timeStringAsMs(podAllocationDelay),
      TimeUnit.MILLISECONDS)
    
    logInfo("Kubernetes cluster scheduler backend started successfully")
  }
  
  override def stop(): Unit = {
    logInfo("Stopping Kubernetes cluster scheduler backend")
    
    try {
      requestExecutorsService.shutdown()
      if (!requestExecutorsService.awaitTermination(10, TimeUnit.SECONDS)) {
        requestExecutorsService.shutdownNow()
      }
      
      executorPodManager.stop()
      kubernetesClient.close()
    } catch {
      case e: Exception =>
        logError("Error stopping Kubernetes cluster scheduler backend", e)
    }
  ...
  override def doRequestTotalExecutors(
      ...
      nodeBlacklist: Set[String]): Future[Boolean] = {
    
    logInfo(s"Requesting $requestedTotal total executors (currently have $totalRegisteredExecutors)")
    totalExpectedExecutors = requestedTotal
    
    Future.successful(true)
  }
  
  override def doKillExecutors(executorIds: Seq[String]): Future[Boolean] = {
    logInfo(s"Killing executors: ${executorIds.mkString(", ")}")
    
    val deletionFutures = executorIds.map { executorId =>
      Future {
        try {
          executorPodManager.deletePod(executorId)
          removeExecutor(executorId, SlaveLost("Executor killed by request"))
          true
        } catch {
          case e: Exception =>
            logError(s"Failed to kill executor $executorId", e)
            false
        }
      }(ThreadUtils.sameThread)
    }
    
    Future.traverse(deletionFutures)(identity).map(_.forall(identity))(ThreadUtils.sameThread)
  }
  
  // 处理Executor请求
  private def handleRequestExecutors(): Unit = {
    val currentRunningCount = totalRegisteredExecutors
    val currentPendingCount = executorPodManager.getPendingPods.size
    val currentTotalCount = currentRunningCount + currentPendingCount
    
    if (currentTotalCount < totalExpectedExecutors) {
      val numExecutorsToRequest = math.min(
        totalExpectedExecutors - currentTotalCount,
        podAllocationSize)
      
      if (numExecutorsToRequest > 0) {
        logInfo(s"Requesting $numExecutorsToRequest new executor pods " +
          s"(running: $currentRunningCount, pending: $currentPendingCount, " +
          s"target: $totalExpectedExecutors)")
        
        requestNewExecutors(numExecutorsToRequest)
      }
    } else if (currentTotalCount > totalExpectedExecutors) {
      val numExecutorsToRemove = currentTotalCount - totalExpectedExecutors
      logInfo(s"Removing $numExecutorsToRemove executor pods to match target $totalExpectedExecutors")
      
      removeExcessExecutors(numExecutorsToRemove)
    }
  }
  
  // 请求新的Executor Pods
  private def requestNewExecutors(numExecutors: Int): Unit = {
    for (i <- 1 to numExecutors) {
      val executorId = s"spark-exec-${UUID.randomUUID().toString.take(8)}"
      
      try {
        val executorPod = buildExecutorPod(executorId)
        executorPodManager.createPod(executorPod)
        logInfo(s"Created executor pod: $executorId")
      } catch {
        case e: Exception =>
          logError(s"Failed to create executor pod $executorId", e)
      }
    }
  }
  
  // 移除多余的Executor
  private def removeExcessExecutors(numToRemove: Int): Unit = {
    val runningExecutors = executorDataMap.keys.toSeq
    val executorsToRemove = runningExecutors.take(numToRemove)
    
    doKillExecutors(executorsToRemove)
  }
  
  // 构建Executor Pod规格
  private def buildExecutorPod(executorId: String): Pod = {
    val executorMemory = sc.conf.getSizeAsMb("spark.executor.memory", "1g")
    val executorCores = sc.conf.getInt("spark.executor.cores", 1)
    val executorImage = kubernetesConf.get("spark.kubernetes.container.image")
    
    val podBuilder = new PodBuilder()
      .withNewMetadata()
        .withName(s"${kubernetesConf.appName}-exec-$executorId")
        .withNamespace(namespace)
        .addToLabels("spark-app-name", kubernetesConf.appName)
        .addToLabels("spark-role", "executor")
        .addToLabels("spark-exec-id", executorId)
      .endMetadata()
      .withNewSpec()
        ...
        .addNewContainer()
          ...
          .endEnv()
          .withNewResources()
            .addToRequests("memory", new Quantity(s"${executorMemory}Mi"))
            .addToRequests("cpu", new Quantity(executorCores.toString))
            .addToLimits("memory", new Quantity(s"${executorMemory}Mi"))
            .addToLimits("cpu", new Quantity(executorCores.toString))
          .endResources()
    ...
    // 添加节点选择器
    val nodeSelector = kubernetesConf.get("spark.kubernetes.node.selector")
    if (nodeSelector.nonEmpty) {
      nodeSelector.foreach { case (key, value) =>
        podBuilder.editSpec().addToNodeSelector(key, value).endSpec()
      }
    }
    
    // 添加Pod亲和性规则
    val podAffinity = kubernetesConf.get("spark.kubernetes.executor.podAffinity")
    if (podAffinity.nonEmpty) {
      // 配置Pod亲和性...
    }
  ...
  private class KubernetesDriverEndpoint(rpcEnv: RpcEnv, sparkProperties: Seq[(String, String)])
    ...
    override def onStart(): Unit = {
      logInfo("Kubernetes driver endpoint started")
    }
    ...
    override def receiveAndReply(context: RpcCallContext): PartialFunction[Any, Unit] = {
      case RegisterExecutor(executorId, executorRef, hostname, cores, logUrls, attributes, resources, resourceProfileId) =>
        if (executorDataMap.contains(executorId)) {
          context.reply(RegisterExecutorFailed(s"Duplicate executor ID: $executorId"))
        } else {
          logInfo(s"Registered executor $executorId on $hostname with $cores cores")
          
          val executorData = new ExecutorData(executorRef, executorRef.address, hostname,
            0, cores, logUrls, attributes, resources, resourceProfileId)
          
          executorDataMap(executorId) = executorData
          totalRegisteredExecutors += 1
          
          // 通知TaskScheduler有新的Executor可用
          makeOffers()
      ...
      case StopExecutors =>
        logInfo("Stopping all executors")
        for (executorId <- executorDataMap.keys) {
...
// Executor Pod管理器
private[spark] class ExecutorPodManager(
    kubernetesClient: KubernetesClient,
    kubernetesConf: KubernetesConf) extends Logging {
  
  private val namespace = kubernetesConf.namespace
  private val appName = kubernetesConf.appName
  
  // Pod状态跟踪
  private val pendingPods = new mutable.HashSet[String]()
  private val runningPods = new mutable.HashMap[String, Pod]()
  
  // Pod监控器
  private val podWatcher = new PodWatcher()
  private var watcherFuture: Future[Watch] = _
  
  def start(): Unit = {
    logInfo("Starting Executor Pod Manager")
    
    // 启动Pod监控
    watcherFuture = Future {
      kubernetesClient.pods()
        .inNamespace(namespace)
        .withLabel("spark-app-name", appName)
        .withLabel("spark-role", "executor")
        .watch(podWatcher)
    }(ThreadUtils.sameThread)
    
    logInfo("Executor Pod Manager started")
  }
  
  def stop(): Unit = {
    logInfo("Stopping Executor Pod Manager")
    
    try {
      watcherFuture.foreach(_.close())
    } catch {
      case e: Exception =>
        logError("Error closing pod watcher", e)
    }
  }
  
  def createPod(pod: Pod): Unit = {
    val executorId = pod.getMetadata.getLabels.get("spark-exec-id")
    pendingPods += executorId
    
    try {
      kubernetesClient.pods().inNamespace(namespace).create(pod)
      logInfo(s"Created pod for executor $executorId")
    } catch {
      case e: Exception =>
        pendingPods -= executorId
        throw e
  ...
  def deletePod(executorId: String): Unit = {
    try {
      kubernetesClient.pods()
        .inNamespace(namespace)
        .withLabel("spark-exec-id", executorId)
        .delete()
      
      pendingPods -= executorId
      runningPods -= executorId
      
      logInfo(s"Deleted pod for executor $executorId")
    } catch {
      case e: Exception =>
        logError(s"Failed to delete pod for executor $executorId", e)
        throw e
    }
  }
  
  def getPendingPods: Set[String] = pendingPods.toSet
  def getRunningPods: Map[String, Pod] = runningPods.toMap
  
  // Pod状态监控器
  private class PodWatcher extends Watcher[Pod] {
    override def eventReceived(action: Watcher.Action, pod: Pod): Unit = {
      ...
      action match {
        case Watcher.Action.ADDED =>
          logInfo(s"Pod $podName added for executor $executorId")
          
        case Watcher.Action.MODIFIED =>
          phase match {
            case "Running" =>
              if (pendingPods.contains(executorId)) {
                pendingPods -= executorId
                runningPods(executorId) = pod
                logInfo(s"Pod $podName is now running for executor $executorId")
              }
              
            case "Failed" | "Succeeded" =>
              pendingPods -= executorId
              runningPods -= executorId
              logWarning(s"Pod $podName finished with phase $phase for executor $executorId")
              
            case _ =>
              logDebug(s"Pod $podName phase changed to $phase for executor $executorId")
          }
          
        case Watcher.Action.DELETED =>
          pendingPods -= executorId
          runningPods -= executorId
          logInfo(s"Pod $podName deleted for executor $executorId")
          
        case Watcher.Action.ERROR =>
          logError(s"Error watching pod $podName for executor $executorId")
      }
    }
    
    override def onClose(cause: KubernetesClientException): Unit = {
      logWarning(s"Pod watcher closed: ${cause.getMessage}")
    }
  }
}
