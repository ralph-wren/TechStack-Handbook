...
- [HBase 技术详解](#hbase-技术详解)
  ...
  - [HBase 基础概念](#hbase-基础概念)
    ...
    - [HBase 核心概念](#hbase-核心概念)
      - [HBase 读写流程原理](#hbase-读写流程原理)
      - [HBase Get查询流程详解](#hbase-get查询流程详解)
      - [HBase Scan查询流程详解](#hbase-scan查询流程详解)
      - [HBase 缓存机制原理](#hbase-缓存机制原理)
      - [HBase 存储格式原理](#hbase-存储格式原理)
      - [Table（表）](#table表)
      - [Row（行）](#row行)
...
```mermaid
graph TD
    A[Client Put请求] --> B[ZooKeeper查询元数据]
    B --> C[获取Region位置]
    C --> D[路由到RegionServer]
    D --> E[RegionServer接收请求]
    E --> F[WAL预写日志]
    F --> G[MemStore内存写入]
    G --> H[返回成功响应]
...
1. **元数据查询**：
   - Client通过ZooKeeper获取表的元数据信息
   - 确定目标RowKey所在的Region和RegionServer
   - 缓存元数据信息，减少后续查询

2. **WAL预写日志**：
   - 在写入MemStore之前，先写入WAL日志
   - WAL日志存储在HDFS上，保证数据持久性
   - 故障恢复时从WAL重放未持久化的数据
...
```mermaid
graph TD
    A[Client Get请求] --> B[ZooKeeper查询元数据]
    B --> C[获取Region位置]
    C --> D[路由到RegionServer]
    D --> E[RegionServer接收请求]
    E --> F[BlockCache查找]
...
```java
// Get查询优化示例
public class GetQueryOptimization {
    
    // 1. 批量Get操作
    public static List<Result> batchGet(Table table, List<String> rowKeys) throws IOException {
    ...
    // 2. 设置缓存策略
    public static Get createCachedGet(String rowKey) {
        Get get = new Get(Bytes.toBytes(rowKey));
    ...
    // 3. 列族和列限定符过滤
    public static Get createFilteredGet(String rowKey, String family, String qualifier) {
        Get get = new Get(Bytes.toBytes(rowKey));
    ...
    // 4. 时间范围过滤
    public static Get createTimeRangeGet(String rowKey, long minTime, long maxTime) {
        Get get = new Get(Bytes.toBytes(rowKey));
    ...
    // 5. 异步Get操作
    public static CompletableFuture<Result> asyncGet(Table table, String rowKey) {
...
| 性能问题 | 原因分析 | 规避策略 |
|---------|----------|----------|
| **缓存未命中** | BlockCache大小不足或数据不在缓存中 | 增加BlockCache大小，预热缓存 |
| **布隆过滤器误判** | 布隆过滤器配置不当 | 选择合适的布隆过滤器类型 |
| **版本过多** | 数据版本数量过多 | 限制版本数量，设置TTL |
| **列族过多** | 跨列族查询性能差 | 合理设计列族，避免跨列族查询 |
| **网络延迟** | 客户端与RegionServer距离远 | 使用本地缓存，批量操作 |
...
```mermaid
graph TD
    A[Client Scan请求] --> B[ZooKeeper查询元数据]
    B --> C[获取Region范围]
    C --> D[路由到多个RegionServer]
    D --> E[RegionServer接收请求]
    E --> F[解析扫描范围]
    F --> G[定位起始Region]
    G --> H[创建Scanner]
...
```java
// Scan查询优化示例
public class ScanQueryOptimization {
    
    // 1. 设置扫描范围
    public static Scan createRangeScan(String startRow, String stopRow) {
        Scan scan = new Scan();
    ...
    // 2. 设置缓存和批量大小
    public static Scan createOptimizedScan() {
        Scan scan = new Scan();
    ...
    // 3. 列族和列限定符过滤
    public static Scan createFilteredScan(String family, String qualifier) {
        Scan scan = new Scan();
    ...
    // 4. 使用过滤器
    public static Scan createFilteredScan() {
        Scan scan = new Scan();
        ...
        SingleColumnValueFilter valueFilter = new SingleColumnValueFilter(
            ...
            CompareFilter.CompareOp.EQUAL, Bytes.toBytes("active"));
    ...
    // 5. 时间范围过滤
    public static Scan createTimeRangeScan(long startTime, long endTime) {
        Scan scan = new Scan();
    ...
    // 6. 分页扫描
    public static Scan createPagedScan(String startRow, int pageSize) {
        Scan scan = new Scan();
        scan.setStartRow(Bytes.toBytes(startRow));
    ...
    // 7. 异步扫描
    public static CompletableFuture<List<Result>> asyncScan(Table table, Scan scan) {
        return CompletableFuture.supplyAsync(() -> {
            try {
...
| 性能问题 | 原因分析 | 规避策略 |
|---------|----------|----------|
| **全表扫描** | 扫描范围过大，没有设置起始和结束行 | 设置合理的扫描范围，使用RowKey前缀 |
| **缓存设置不当** | Caching和Batch大小不合理 | 根据数据大小设置合适的缓存参数 |
| **过滤器效率低** | 过滤器选择不当，计算复杂 | 使用高效的过滤器，避免全表扫描 |
| **网络传输量大** | 返回数据过多，没有列过滤 | 只选择需要的列，使用列族过滤 |
| **Region跳跃** | 扫描跨越多个Region，网络开销大 | 优化RowKey设计，减少Region跳跃 |
| **内存使用过多** | 一次性加载大量数据到内存 | 使用分页扫描，控制内存使用 |
...
public class AdvancedOptimization {
    ...
    // 1. 使用协处理器进行服务端过滤
    public static Scan createCoprocessorScan() {
        Scan scan = new Scan();
        scan.setAttribute("coprocessor", "org.apache.hadoop.hbase.coprocessor.AggregateImplementation");
        return scan;
    }
    
    // 2. 使用布隆过滤器优化
    public static void configureBloomFilter(TableDescriptor tableDesc) {
        HColumnDescriptor cf = new HColumnDescriptor("cf");
        cf.setBloomFilterType(BloomType.ROWCOL); // 行列级布隆过滤器
        tableDesc.addFamily(cf);
    }
    
    // 3. 使用压缩减少IO
    public static void configureCompression(TableDescriptor tableDesc) {
        HColumnDescriptor cf = new HColumnDescriptor("cf");
    ...
    // 4. 使用TTL自动清理数据
    public static void configureTTL(TableDescriptor tableDesc) {
        HColumnDescriptor cf = new HColumnDescriptor("cf");
        cf.setTimeToLive(86400); // 24小时TTL
        tableDesc.addFamily(cf);
    }
    
    // 5. 监控扫描性能
    public static void monitorScanPerformance(Table table, Scan scan) {
        ...
        try {
            ...
            System.out.println("Scan completed: " + count + " rows in " + (endTime - startTime) + "ms");
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
...
| 指标类型 | 指标名称 | 监控目的 | 优化建议 |
|---------|----------|----------|----------|
| **响应时间** | 平均响应时间 | 监控查询性能 | 优化缓存和索引 |
| **吞吐量** | 每秒查询数 | 监控处理能力 | 增加并发和批量操作 |
| **缓存命中率** | BlockCache命中率 | 监控缓存效果 | 调整缓存大小和策略 |
| **网络IO** | 数据传输量 | 监控网络开销 | 减少数据传输，使用压缩 |
| **内存使用** | 内存使用率 | 监控资源使用 | 优化内存配置，控制批量大小 |
...
```mermaid
graph TD
    A[客户端缓存] --> B[RegionServer BlockCache]
    B --> C[RegionServer MemStore]
    C --> D[HDFS HFile]
    
    E[ZooKeeper元数据缓存] --> F[Master元数据缓存]
    F --> G[RegionServer元数据缓存]
    
    H[客户端连接池] --> I[RegionServer连接池]
...
| 缓存层级 | 一致性挑战 | 影响范围 | 严重程度 |
|---------|----------|----------|----------|
| **客户端缓存** | 元数据过期，连接池状态不一致 | 单个客户端 | 中等 |
| **BlockCache** | 数据块版本不一致，脏数据 | 单个RegionServer | 高 |
| **MemStore** | 未刷写数据丢失，版本冲突 | 单个Region | 高 |
| **元数据缓存** | Region位置信息过期 | 整个集群 | 高 |
| **连接池缓存** | 连接状态不一致 | 网络层面 | 中等 |
...
public class CacheConsistencyHandler {
    ...
    // 1. 客户端缓存失效处理
    public static class ClientCacheInvalidation {
        
        // 元数据缓存失效
        public static void invalidateMetadataCache(Connection connection) {
            try {
                // 清除本地元数据缓存
                connection.clearRegionLocationCache();
                // 重新获取元数据
                connection.getTable(TableName.valueOf("test_table"));
            } catch (IOException e) {
                log.error("Failed to invalidate metadata cache", e);
            }
        }
        
        // 连接池状态检查
        public static boolean checkConnectionHealth(Connection connection) {
            try {
                // 测试连接是否有效
        ...
        // 自动重连机制
        public static Connection reconnectIfNeeded(Connection connection) {
            if (!checkConnectionHealth(connection)) {
                try {
                    connection.close();
    ...
    // 2. BlockCache一致性处理
    public static class BlockCacheConsistency {
        
        // 缓存版本检查
        public static boolean validateCacheVersion(Result result, long expectedVersion) {
            long actualVersion = result.getColumnLatestCell(
                Bytes.toBytes("cf"), Bytes.toBytes("version")).getTimestamp();
            return actualVersion >= expectedVersion;
        }
        
        // 强制刷新缓存
        public static void forceCacheRefresh(Table table, String rowKey) {
            try {
                Get get = new Get(Bytes.toBytes(rowKey));
                get.setCacheBlocks(false); // 禁用缓存
                Result result = table.get(get);
                // 手动更新缓存
                updateLocalCache(rowKey, result);
            } catch (IOException e) {
        ...
        // 缓存预热策略
        public static void warmupCache(Table table, List<String> hotRowKeys) {
            for (String rowKey : hotRowKeys) {
                try {
                    Get get = new Get(Bytes.toBytes(rowKey));
                    get.setCacheBlocks(true);
                    table.get(get); // 预加载到缓存
                } catch (IOException e) {
                    log.warn("Failed to warmup cache for rowKey: " + rowKey, e);
                }
            }
        }
    }
    ...
    // 3. MemStore一致性处理
    public static class MemStoreConsistency {
        
        // 检查MemStore状态
        public static boolean checkMemStoreStatus(RegionInfo regionInfo) {
            try {
                // 检查MemStore是否已刷写
                long memStoreSize = getMemStoreSize(regionInfo);
                return memStoreSize < getMemStoreFlushThreshold();
            } catch (Exception e) {
                log.error("Failed to check MemStore status", e);
                return false;
            }
        }
        
        // 强制刷写MemStore
        public static void forceMemStoreFlush(Admin admin, TableName tableName) {
            try {
                admin.flush(tableName);
                log.info("Forced MemStore flush for table: " + tableName);
            } catch (IOException e) {
                log.error("Failed to force MemStore flush", e);
            }
        }
        
        // 版本冲突检测
        public static boolean detectVersionConflict(Result result, long writeTimestamp) {
            for (Cell cell : result.rawCells()) {
                if (cell.getTimestamp() > writeTimestamp) {
                    log.warn("Version conflict detected: cell timestamp {} > write timestamp {}", 
                            cell.getTimestamp(), writeTimestamp);
                    return true;
                }
            }
            return false;
    ...
    // 4. 元数据缓存一致性处理
    public static class MetadataCacheConsistency {
        
        // Region位置缓存失效
        public static void invalidateRegionLocationCache(Connection connection, TableName tableName) {
            try {
                connection.clearRegionLocationCache(tableName);
                log.info("Invalidated region location cache for table: " + tableName);
            } catch (Exception e) {
                log.error("Failed to invalidate region location cache", e);
            }
        }
        
        // 检查Region分裂状态
        public static boolean checkRegionSplitStatus(Admin admin, TableName tableName) {
            try {
                List<RegionInfo> regions = admin.getRegions(tableName);
                for (RegionInfo region : regions) {
                    if (region.isSplit()) {
                        log.warn("Region is in split state: " + region.getRegionNameAsString());
                        return true;
                    }
                }
                return false;
            } catch (IOException e) {
                log.error("Failed to check region split status", e);
                return false;
            }
        }
        
        // 更新Region位置信息
        public static void updateRegionLocation(Connection connection, TableName tableName) {
            try {
                // 重新获取Region位置信息
                Table table = connection.getTable(tableName);
                table.get(new Get(Bytes.toBytes("dummy"))); // 触发元数据更新
                table.close();
            } catch (IOException e) {
                log.error("Failed to update region location", e);
            }
        }
    }
}
...
| 处理策略 | 适用场景 | 实现方式 | 性能影响 |
|---------|----------|----------|----------|
| **主动失效** | 数据更新频繁 | 写入时主动清除相关缓存 | 中等 |
| **版本检查** | 版本敏感数据 | 比较缓存版本与数据版本 | 低 |
| **强制刷新** | 缓存数据过期 | 禁用缓存重新获取数据 | 高 |
| **增量更新** | 部分数据变更 | 只更新变更的数据块 | 低 |
| **异步同步** | 非实时一致性要求 | 后台异步更新缓存 | 最低 |
...
public class CacheConsistencyMonitor {
    ...
    private static final Logger log = LoggerFactory.getLogger(CacheConsistencyMonitor.class);
    
    // 监控指标
    private final AtomicLong cacheMissCount = new AtomicLong(0);
    private final AtomicLong versionConflictCount = new AtomicLong(0);
    private final AtomicLong metadataStaleCount = new AtomicLong(0);
    
    // 1. 缓存命中率监控
    public void monitorCacheHitRate(Table table, String rowKey) {
        long startTime = System.currentTimeMillis();
        try {
            Get get = new Get(Bytes.toBytes(rowKey));
            Result result = table.get(get);
            ...
            // 记录缓存性能指标
            recordCacheMetrics(endTime - startTime, result);
        } catch (IOException e) {
            cacheMissCount.incrementAndGet();
            log.error("Cache miss for rowKey: " + rowKey, e);
        }
    }
    
    // 2. 版本一致性检查
    public boolean checkVersionConsistency(Result result, long expectedVersion) {
        try {
            Cell versionCell = result.getColumnLatestCell(
                Bytes.toBytes("cf"), Bytes.toBytes("version"));
            if (versionCell != null) {
                long actualVersion = versionCell.getTimestamp();
                if (actualVersion != expectedVersion) {
                    versionConflictCount.incrementAndGet();
                    log.warn("Version inconsistency detected: expected={}, actual={}", 
            ...
            log.error("Failed to check version consistency", e);
            return false;
        }
    }
    
    // 3. 元数据一致性检查
    public boolean checkMetadataConsistency(Connection connection, TableName tableName) {
        try {
            // 检查Region位置信息是否最新
            Table table = connection.getTable(tableName);
            List<RegionInfo> regions = table.getRegionLocator().getAllRegionLocations()
                .stream()
            ...
            for (RegionInfo region : regions) {
                if (region.isSplit() || region.isOffline()) {
                    metadataStaleCount.incrementAndGet();
                    log.warn("Stale metadata detected for region: " + region.getRegionNameAsString());
                    return false;
                }
            }
            return true;
        } catch (IOException e) {
            log.error("Failed to check metadata consistency", e);
            return false;
        }
    }
    
    // 4. 生成监控报告
    public CacheConsistencyReport generateReport() {
        return CacheConsistencyReport.builder()
            .cacheMissCount(cacheMissCount.get())
            .versionConflictCount(versionConflictCount.get())
            .metadataStaleCount(metadataStaleCount.get())
    ...
    @Data
    @Builder
    public static class CacheConsistencyReport {
        private long cacheMissCount;
        private long versionConflictCount;
        private long metadataStaleCount;
        private long timestamp;
        
        public double getCacheHitRate() {
            long totalRequests = cacheMissCount + versionConflictCount + metadataStaleCount;
            return totalRequests > 0 ? (double) (totalRequests - cacheMissCount) / totalRequests : 1.0;
        }
    }
}
```
...
| 实践类别 | 具体措施 | 预期效果 | 注意事项 |
|---------|----------|----------|----------|
| **设计层面** | 合理设置缓存TTL，使用版本号 | 减少不一致概率 | 平衡一致性和性能 |
| **实现层面** | 实现重试机制，使用乐观锁 | 提高数据可靠性 | 避免死锁和性能下降 |
| **监控层面** | 实时监控缓存状态，设置告警 | 及时发现和处理问题 | 避免监控开销过大 |
| **运维层面** | 定期清理过期缓存，重启服务 | 保持系统健康状态 | 选择低峰期执行 |
| **测试层面** | 压力测试缓存一致性，模拟故障 | 验证系统健壮性 | 覆盖各种异常场景 |

**故障恢复策略**：

```java
// 故障恢复策略
public class CacheRecoveryStrategy {
    
    // 1. 缓存重建策略
    public static void rebuildCache(Table table, List<String> criticalRowKeys) {
        log.info("Starting cache rebuild for {} critical rowKeys", criticalRowKeys.size());
        
        for (String rowKey : criticalRowKeys) {
            try {
                // 清除旧缓存
                clearCacheForRowKey(rowKey);
                ...
                // 验证数据完整性
                if (validateDataIntegrity(result)) {
                    log.info("Successfully rebuilt cache for rowKey: " + rowKey);
                } else {
                    log.error("Data integrity check failed for rowKey: " + rowKey);
                }
            } catch (IOException e) {
                log.error("Failed to rebuild cache for rowKey: " + rowKey, e);
            }
        }
    }
    
    // 2. 降级策略
    public static Result getWithFallback(Table table, String rowKey) {
        try {
            // 尝试从缓存获取
            Get get = new Get(Bytes.toBytes(rowKey));
            get.setCacheBlocks(true);
            ...
            log.warn("Cache access failed, falling back to direct read", e);
            
            try {
                // 降级到直接读取
                Get fallbackGet = new Get(Bytes.toBytes(rowKey));
                fallbackGet.setCacheBlocks(false);
                return table.get(fallbackGet);
            } catch (IOException fallbackException) {
                log.error("Fallback read also failed", fallbackException);
                throw new RuntimeException("Both cache and direct read failed", fallbackException);
            }
        }
    }
    
    // 3. 数据完整性验证
    private static boolean validateDataIntegrity(Result result) {
        if (result == null || result.isEmpty()) {
            return false;
        }
        
        // 检查关键字段是否存在
        Cell nameCell = result.getColumnLatestCell(Bytes.toBytes("cf"), Bytes.toBytes("name"));
        Cell timestampCell = result.getColumnLatestCell(Bytes.toBytes("cf"), Bytes.toBytes("timestamp"));
        
        return nameCell != null && timestampCell != null;
    }
    
    // 4. 缓存预热策略
    public static void warmupCache(Table table, List<String> hotRowKeys, int batchSize) {
        log.info("Starting cache warmup for {} rowKeys with batch size {}", 
                hotRowKeys.size(), batchSize);
        
        List<List<String>> batches = partitionList(hotRowKeys, batchSize);
        ...
        for (List<String> batch : batches) {
            try {
                ...
                log.info("Warmed up cache for batch of {} rowKeys", batch.size());
                
                // 验证预热结果
                for (int i = 0; i < results.length; i++) {
                    if (results[i] == null || results[i].isEmpty()) {
                        log.warn("Cache warmup failed for rowKey: " + batch.get(i));
                    }
                }
            } catch (IOException e) {
                log.error("Failed to warmup cache for batch", e);
            }
        }
    }
...
| 缓存类型 | 位置 | 作用 | 淘汰策略 | 配置参数 |
|---------|------|------|----------|----------|
| **MemStore** | 内存 | 写操作缓存 | 刷写到HFile | hbase.regionserver.global.memstore.size |
| **BlockCache** | 内存 | 读操作缓存 | LRU算法 | hfile.block.cache.size |
| **L1Cache** | Region级别 | 热点数据缓存 | LRU算法 | hbase.regionserver.blockcache.l1.size |
| **L2Cache** | 全局共享 | 跨Region缓存 | LRU算法 | hbase.regionserver.blockcache.l2.size |
...
public class CacheConfiguration {
    ...
    public static void configureCache(Configuration conf) {
        // BlockCache配置 - 读缓存
        conf.set("hfile.block.cache.size", "0.4"); // 占堆内存40%
        conf.set("hbase.regionserver.blockcache.l1.size", "0.1"); // L1缓存10%
        conf.set("hbase.regionserver.blockcache.l2.size", "0.3"); // L2缓存30%
        
        // MemStore配置 - 写缓存
        conf.set("hbase.regionserver.global.memstore.size", "0.4"); // 占堆内存40%
        conf.set("hbase.regionserver.global.memstore.size.lower.limit", "0.35"); // 35%触发刷写
        
        // 缓存类型配置
        conf.set("hbase.regionserver.blockcache.impl", 
                "org.apache.hadoop.hbase.io.hfile.LruBlockCache");
    }
    
    // 缓存预热
    public static void warmUpCache(Table table, String rowKey) throws IOException {
        Get get = new Get(Bytes.toBytes(rowKey));
        get.setMaxVersions(1); // 只获取最新版本
        table.get(get); // 触发缓存加载
    }
}
```
...
| 组件 | 位置 | 大小 | 作用 | 特点 |
|------|------|------|------|------|
| **Data Block** | 文件主体 | 默认64KB | 存储Key-Value数据 | 可压缩，支持随机访问 |
| **Meta Block** | 数据块后 | 可变 | 存储布隆过滤器等元数据 | 提高查询效率 |
| **File Info** | 文件末尾 | 固定 | 文件级别元数据 | 包含压缩类型等信息 |
| **Data Index** | 文件末尾 | 可变 | 数据块索引 | 支持二分查找 |
| **Meta Index** | 文件末尾 | 可变 | 元数据块索引 | 快速定位元数据 |
| **Trailer** | 文件末尾 | 固定 | 文件头信息 | 包含各组件偏移量 |
...
graph TD
    ...
    J[Minor Compaction] --> K[小文件合并]
    K --> L[中等文件]
    
    M[Major Compaction] --> N[所有文件合并]
    N --> O[大文件]
    O --> P[删除小文件]
...
public class StorageOptimization {
    ...
    public static void configureStorage(Configuration conf) {
        // 压缩配置
        conf.set("hbase.regionserver.codecs", "snappy,gz,lzo");
        conf.set("hbase.regionserver.default.compression", "snappy");
        
        // 布隆过滤器配置
        conf.set("hbase.regionserver.bloom.enabled", "true");
        conf.set("hbase.regionserver.bloom.type", "ROW");
        
        // 块大小配置
        conf.set("hfile.block.size", "65536"); // 64KB
        
        // 合并配置
        conf.set("hbase.hstore.compaction.min", "3");
        conf.set("hbase.hstore.compaction.max", "10");
        conf.set("hbase.hstore.compaction.ratio", "1.2");
    }
    
    // 预分区策略
    public static byte[][] createPreSplits(int numRegions) {
...
public class RowKeyDesign {
    ...
    // 1. 哈希策略 - 避免热点
    public static String createHashRowKey(String originalKey) {
        int hash = Math.abs(originalKey.hashCode());
        return String.format("%03d", hash % 1000) + "_" + originalKey;
    }
    
    // 2. 反转策略 - 时间戳反转
    public static String createReverseTimestampRowKey(long timestamp) {
        String timestampStr = String.format("%020d", timestamp);
        return new StringBuilder(timestampStr).reverse().toString();
    }
    
    // 3. 加盐策略 - 随机前缀
    public static String createSaltRowKey(String originalKey) {
        int salt = new Random().nextInt(1000);
        return String.format("%03d", salt) + "_" + originalKey;
    }
    
    // 4. 复合策略 - 多字段组合
    public static String createCompositeRowKey(String userId, long timestamp, String action) {
        return userId + "_" + String.format("%020d", timestamp) + "_" + action;
    }
    
    // 5. 预分区策略 - 根据RowKey范围分区
    public static byte[][] createPreSplitsForUserData() {
        byte[][] splits = new byte[9][];
        for (int i = 1; i <= 9; i++) {
            splits[i-1] = Bytes.toBytes(String.format("user_%03d", i * 100));
        }
        return splits;
    }
}
...
```java
// Region分裂配置
public class RegionSplitConfiguration {
    
    public static void configureRegionSplit(Configuration conf) {
        // 分裂阈值配置
        conf.set("hbase.hregion.max.filesize", "10737418240"); // 10GB
        conf.set("hbase.regionserver.region.split.policy", 
                "org.apache.hadoop.hbase.regionserver.SteppingSplitPolicy");
        
        // 分裂策略配置
        conf.set("hbase.regionserver.region.split.minimum.size", "134217728"); // 128MB
        conf.set("hbase.regionserver.region.split.maximum.size", "10737418240"); // 10GB
        
        // 分裂检查间隔
        conf.set("hbase.regionserver.region.split.check.interval", "10000"); // 10秒
    }
    
    // 手动分裂Region
    public static void splitRegion(Admin admin, String regionName) throws IOException {
        admin.splitRegion(Bytes.toBytes(regionName));
    }
    
    // 预分区策略
    public static void createTableWithPreSplits(Admin admin, String tableName, 
...
```mermaid
graph TD
    A[负载均衡检查] --> B{Region分布是否均衡}
    B -->|不均衡| C[计算最优分布]
    B -->|均衡| D[无需调整]
    
    C --> E[选择迁移Region]
    E --> F[选择目标RegionServer]
    F --> G[执行Region迁移]
    G --> H[更新元数据]
    H --> I[验证迁移结果]
...
| 优化策略 | 目的 | 实现方式 |
|---------|------|----------|
| **预分区** | 避免自动分裂 | 创建表时指定分割点 |
| **大小控制** | 平衡读写性能 | 设置合适的分裂阈值 |
| **负载均衡** | 均匀分布负载 | 定期执行负载均衡 |
| **热点处理** | 避免热点Region | RowKey设计和预分区 |
| **监控告警** | 及时发现问题 | 监控Region数量和大小 |
...
<!-- ```java -->
// RegionServer性能配置
public class RegionServerOptimization {
    
    public static void configureRegionServer(Configuration conf) {
        // 内存配置
        conf.set("hbase.regionserver.global.memstore.size", "0.4"); // 40%堆内存
        conf.set("hfile.block.cache.size", "0.4"); // 40%堆内存
        ...
        // 线程池配置
        conf.set("hbase.regionserver.handler.count", "30"); // 处理线程数
        conf.set("hbase.regionserver.msginterval", "3000"); // 消息间隔
    ...
    // RegionServer监控
    public static class RegionServerMonitoring {
        ...
        // 监控关键指标
        public static Map<String, Object> getKeyMetrics(ServerName serverName) throws IOException {
            Map<String, Object> metrics = new HashMap<>();
            
            // 内存使用率
            metrics.put("memstoreSize", getMemStoreSize(serverName));
            metrics.put("blockCacheSize", getBlockCacheSize(serverName));
            
            // 请求统计
            metrics.put("readRequests", getReadRequests(serverName));
            metrics.put("writeRequests", getWriteRequests(serverName));
            
            // Region统计
            metrics.put("regionCount", getRegionCount(serverName));
            metrics.put("storeCount", getStoreCount(serverName));
...
```mermaid
graph TD
    A[故障检测] --> B[ZooKeeper心跳超时]
    B --> C[Master检测故障]
    C --> D[标记RegionServer为故障]
    D --> E[重新分配Region]
    E --> F[从WAL恢复数据]
    F --> G[Region重新上线]
    G --> H[验证数据一致性]
...
| 指标类型 | 指标名称 | 监控目的 | 告警阈值 |
|---------|----------|----------|----------|
| **内存指标** | MemStore大小 | 监控写缓存使用 | >80%堆内存 |
| **内存指标** | BlockCache大小 | 监控读缓存使用 | >80%堆内存 |
| **性能指标** | 请求延迟 | 监控响应时间 | >100ms |
| **性能指标** | 请求吞吐量 | 监控处理能力 | <1000 req/s |
| **资源指标** | Region数量 | 监控负载分布 | >100个 |
| **资源指标** | 磁盘使用率 | 监控存储空间 | >80% |
```
...
**写入流程详解**：
1. **客户端请求**：Client发起Put请求
2. **元数据查询**：通过ZooKeeper获取Region位置信息
3. **路由转发**：将请求发送到对应的RegionServer
4. **WAL写入**：先写入Write-Ahead Log，保证数据持久性
5. **内存写入**：数据写入MemStore内存缓冲区
6. **响应返回**：向客户端返回写入成功

**读取流程详解**：
1. **客户端请求**：Client发起Get请求
2. **元数据查询**：通过ZooKeeper获取Region位置信息
3. **路由转发**：将请求发送到对应的RegionServer
4. **缓存查找**：首先在BlockCache中查找数据块
5. **内存查找**：在MemStore中查找最新写入的数据
6. **磁盘查找**：在HFile中查找持久化数据
7. **数据合并**：合并多个版本的数据
8. **响应返回**：向客户端返回最终结果
...
| 组件 | 位置 | 作用 | 大小 |
|------|------|------|------|
| **Data Block** | 文件主体 | 存储Key-Value数据 | 默认64KB |
| **Meta Block** | 数据块后 | 存储布隆过滤器等元数据 | 可变 |
| **File Info** | 文件末尾 | 存储文件级别元数据 | 固定 |
| **Data Index** | 文件末尾 | 数据块索引，支持随机访问 | 可变 |
| **Meta Index** | 文件末尾 | 元数据块索引 | 可变 |
| **Trailer** | 文件末尾 | 文件头信息，包含各组件偏移量 | 固定 |
...
public class HFileStructure {
    ...
    public static class DataBlock {
        private byte[] key;           // RowKey
        private byte[] family;        // 列族
        private byte[] qualifier;     // 列限定符
        private long timestamp;       // 时间戳
        private byte[] value;         // 值
        private byte keyType;         // 键类型（PUT/DELETE）
    }
    
    // 索引结构
    public static class BlockIndex {
        private byte[] firstKey;      // 块中第一个Key
        private long offset;          // 块在文件中的偏移量
        private int size;             // 块大小
        private byte[] bloomFilter;   // 布隆过滤器
    }
}
```
...
1. **压缩存储**：支持GZ、LZO、SNAPPY、ZSTD等压缩算法
2. **布隆过滤器**：快速判断数据是否存在，减少磁盘IO
3. **块索引**：支持随机访问，提高查询性能
4. **版本控制**：支持多版本数据存储
5. **合并压缩**：定期合并小文件，优化存储结构
...
| 层次 | 组件 | 作用 | 特点 |
|------|------|------|------|
| **应用层** | Client | 客户端请求 | 提供API接口 |
| **服务层** | RegionServer | 处理请求 | 协调读写操作 |
| **逻辑层** | Region | 数据分区 | 管理表的一部分数据 |
| **内存层** | MemStore | 写缓存 | 内存缓冲区，提高写入性能 |
| **缓存层** | BlockCache | 读缓存 | 缓存数据块，提高读取性能 |
| **存储层** | HFile | 持久化存储 | LSM树结构，支持高效读写 |
| **日志层** | WAL | 预写日志 | 保证数据一致性和持久性 |
...
| 缓存类型 | 位置 | 作用 | 淘汰策略 | 大小配置 |
|---------|------|------|----------|----------|
| **MemStore** | 内存 | 写操作缓存 | 刷写到HFile | 默认128MB |
| **BlockCache** | 内存 | 读操作缓存 | LRU算法 | 默认40%堆内存 |
| **L1Cache** | Region级别 | 热点数据缓存 | LRU算法 | 可配置 |
| **L2Cache** | 全局共享 | 跨Region缓存 | LRU算法 | 可配置 |
...
// 缓存类型配置
conf.set("hfile.block.cache.size", "0.4"); // BlockCache占堆内存40%
conf.set("hbase.regionserver.global.memstore.size", "0.4"); // MemStore占堆内存40%
```

**17. HBase的MemStore刷写机制？**
**答案**：MemStore刷写触发条件：大小超过阈值（默认128MB）、全局MemStore大小超过限制、手动触发、RegionServer关闭。刷写过程：将内存数据写入HFile，清空MemStore。

**18. HBase的BlockCache策略？**
