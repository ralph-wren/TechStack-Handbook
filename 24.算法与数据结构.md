...
拓扑排序是一种对有向无环图(DAG)进行排序的算法，使得对于图中的每一条有向边(u, v)，节点u在排序中都出现在节点v之前。拓扑排序常用于任务调度、编译依赖分析等场景。
...
拓扑排序有两种主要实现方法：Kahn算法(基于入度)和深度优先搜索(DFS)算法。

#### Kahn算法(基于入度)

Kahn算法的基本思想是：
1. 找出所有入度为0的节点，加入队列
2. 从队列中取出一个节点，将其加入结果序列
3. 将该节点的所有邻接节点的入度减1
4. 如果减1后邻接节点的入度变为0，则将其加入队列
5. 重复步骤2-4，直到队列为空
...
public class TopologicalSort {
    ...
    public List<Integer> topologicalSortKahn(int numVertices, int[][] edges) {
        ...
        if (result.size() != numVertices) {
            return new ArrayList<>(); // 存在环，无法完成拓扑排序
        }
...
DFS算法的基本思想是：
1. 对图中每个未访问的节点执行DFS
2. 在DFS的过程中，标记节点为"正在访问"
3. 递归访问所有邻接节点
4. 当节点的所有邻接节点都被访问后，将节点标记为"已访问"，并将其加入结果序列的头部
5. 如果在DFS过程中发现一个"正在访问"的节点，则说明存在环
...
public class TopologicalSortDFS {
    ...
    private boolean dfs(List<List<Integer>> graph, int node, int[] visited, LinkedList<Integer> result) {
        // 标记为正在访问
        visited[node] = 1;
...
1. **任务调度**：在项目管理中，任务之间可能存在依赖关系，拓扑排序可以确定任务的执行顺序。

2. **编译依赖分析**：在编译系统中，模块之间的依赖关系可以用有向图表示，拓扑排序可以确定编译顺序。

3. **课程安排**：在学校课程安排中，某些课程可能是其他课程的先修课，拓扑排序可以帮助确定合理的学习顺序。

4. **数据处理流水线**：在数据处理系统中，不同的处理阶段可能有依赖关系，拓扑排序可以确定数据处理的流程。

```java
/**
 * 课程安排问题
 * 给定n门课程和它们的先修关系，判断是否可能完成所有课程
 */
...
拓扑排序是图论中的重要算法，它在解决依赖关系问题时非常有用。需要注意的是，拓扑排序只适用于有向无环图，如果图中存在环，则无法得到一个合法的拓扑序列。

### 字符串处理

字符串处理是算法中的重要组成部分，涉及到字符串的转换、解析、格式化等操作。高效的字符串处理算法对于文本处理、自然语言处理等领域至关重要。
...
字符串的基本操作包括比较、子串提取、分割与连接等，这些是更复杂字符串算法的基础。
...
public class StringBasicOperations {
     ...
     * 空间复杂度：O(n)
     */
    public List<String> processWords(String text, String delimiter) {
...
编辑距离是衡量两个字符串差异的一种方法，定义为将一个字符串转换为另一个字符串所需的最少操作次数（插入、删除、替换）。
...
字符串压缩是减少字符串存储空间的技术，常用于文本压缩和数据传输。
...
public class StringCompression {
     ...
     * 空间复杂度：O(n)
     */
    public String compressByCount(String s) {
        if (s == null || s.length() <= 1) {
            return s;
        }
...
正则表达式是处理字符串的强大工具，可以用于模式匹配、验证、提取和替换。
...
public class RegexExamples {
     ...
     * 替换敏感词
     */
    public String censorText(String text, List<String> sensitiveWords) {
...
字符串处理算法在许多实际应用中都有广泛的应用，例如：

1. **文本编辑器**：实现查找、替换、拼写检查等功能。

2. **自然语言处理**：分词、词频统计、文本分类等。

3. **搜索引擎**：关键词匹配、相似度计算等。

4. **生物信息学**：DNA序列比对、蛋白质序列分析等。

5. **数据解析**：解析JSON、XML、CSV等格式的数据。
...
public class TextSimilarity {
    ...
    public double cosineSimilarity(String text1, String text2) {
        ...
        // 构建词汇表
        Set<String> vocabulary = new HashSet<>(freq1.keySet());
        vocabulary.addAll(freq2.keySet());
        
        // 计算点积
        double dotProduct = 0;
        for (String term : vocabulary) {
            dotProduct += freq1.getOrDefault(term, 0) * freq2.getOrDefault(term, 0);
        }
...
字符串处理是算法中的基础且重要的部分，掌握高效的字符串处理技术对于解决各种实际问题都有很大帮助。

## 回溯算法

回溯算法是一种通过探索所有可能的解决方案来找到所需解的算法。它通常用于解决约束满足问题，如组合问题、排列问题、子集问题等。
...
回溯算法的核心思想是从一个初始状态出发，暴力搜索所有可能的解决方案，当发现当前路径不可能是正确的解时，就"回溯"到上一个状态，尝试其他可能的路径。
...
public class BacktrackingAlgorithm {
     ...
     * @param 结果集
     */
    private void backtrack(State state, List<Choice> choices, List<Solution> results) {
        // 判断是否到达终止条件
        if (isEndState(state)) {
            results.add(createSolution(state));
            return;
        }
        ...
        for (Choice choice : choices) {
            ...
            if (isValid(state, choice)) {
                ...
                // 递归进入下一层决策树
                backtrack(state, getNextChoices(state, choices), results);
...
回溯算法的时间复杂度通常是指数级的，但通过剪枝技术可以大幅减少搜索空间。

### 经典回溯问题

#### 1. N皇后问题

N皇后问题是在N×N的棋盘上放置N个皇后，使得它们互不攻击（即任意两个皇后都不能处于同一行、同一列或同一斜线上）。
...
public class NQueens {
    ...
    private void backtrack(char[][] board, int row, List<List<String>> results) {
        // 终止条件：所有行都放置了皇后
        if (row == board.length) {
            results.add(createSolution(board));
            return;
        }
...
给定一个无重复元素的数组和一个目标数，找出数组中所有可以使数字和为目标数的组合。
...
public class CombinationSum {
    ...
    private void backtrack(int[] candidates, int remain, int start, List<Integer> current, List<List<Integer>> results) {
        ...
        if (remain < 0) {
            ...
            results.add(new ArrayList<>(current)); // 找到一个解
            return;
        }
...
回溯算法是解决组合优化问题的强大工具，虽然其时间复杂度较高，但通过合理的剪枝策略和问题建模，可以有效地解决许多实际问题。