...
- [ClickHouse](#clickhouse)
  ...
  - [数据操作](#数据操作)
    ...
    - [Map操作](#map操作)
      ...
      - [Map聚合操作](#map聚合操作)
    - [数组操作](#数组操作)
      - [数组展开函数（类似Explode）](#数组展开函数类似explode)
        - [基本数组展开](#基本数组展开)
        - [复杂数组展开场景](#复杂数组展开场景)
        - [高级数组展开操作](#高级数组展开操作)
        - [数组展开性能优化](#数组展开性能优化)
      - [其他数组操作函数](#其他数组操作函数)
      - [数组聚合函数（类似Hive聚合函数）](#数组聚合函数类似hive聚合函数)
      ...
      - [其他数组操作函数](#其他数组操作函数-1)
      - [ClickHouse与Hive聚合函数对比和迁移示例](#clickhouse与hive聚合函数对比和迁移示例)
        - [Hive到ClickHouse常用聚合函数迁移对照表](#hive到clickhouse常用聚合函数迁移对照表)
        - [综合实际应用示例](#综合实际应用示例)
      - [数组集合操作](#数组集合操作)
...
**arrayJoin函数**是ClickHouse中类似于Spark中`explode`函数的核心数组展开功能，它将数组的每个元素转换为独立的行。
...
-- 展开字符串数组
SELECT arrayJoin(['apple', 'banana', 'cherry']) AS fruit;
-- 结果:
-- apple
-- banana
-- cherry
...
-- 展开嵌套数组（只展开第一层）
SELECT arrayJoin([[1, 2], [3, 4], [5, 6]]) AS nested_array;
-- 结果:
-- [1, 2]
-- [3, 4] 
-- [5, 6]
...
-- 完全展开嵌套数组
SELECT arrayJoin(arrayJoin([[1, 2], [3, 4], [5, 6]])) AS value;
-- 结果:
-- 1
-- 2
-- 3
...
FROM (
    SELECT 
        1 AS id, 
        'Product A' AS name, 
        ['electronics', 'mobile', 'smartphone'] AS tags
    UNION ALL
    SELECT 
        2 AS id, 
        'Product B' AS name, 
        ['home', 'kitchen', 'appliance'] AS tags
);
-- 结果:
-- 1 | Product A | electronics
-- 1 | Product A | mobile
-- 1 | Product A | smartphone
-- 2 | Product B | home
-- 2 | Product B | kitchen
-- 2 | Product B | appliance

-- 多个数组同时展开（必须长度相同）
SELECT 
    arrayJoin(['A', 'B', 'C']) AS letter,
    arrayJoin([1, 2, 3]) AS number;
-- 结果:
-- A | 1
-- B | 2
-- C | 3
...
FROM (
    SELECT arrayJoin(['electronics', 'mobile', 'smartphone', 'electronics', 'laptop']) AS tag
)
GROUP BY tag
...
SELECT 
    user_id,
    arrayJoin(extractAll(events, '"event_type":"([^"]+)"')) AS event_type
FROM user_events
WHERE events != '';

-- 展开Map的keys和values
SELECT 
    arrayJoin(mapKeys(user_attributes)) AS attribute_name,
    arrayJoin(mapValues(user_attributes)) AS attribute_value
FROM user_profiles;

-- 条件性数组展开
SELECT 
    id,
    arrayJoin(arrayFilter(x -> x > 10, numbers)) AS filtered_number
FROM (
    SELECT 
        1 AS id,
        [5, 15, 8, 20, 12] AS numbers
);
-- 结果:
-- 1 | 15
-- 1 | 20
...
#### ClickHouse与Hive聚合函数对比和迁移示例

##### Hive到ClickHouse常用聚合函数迁移对照表
