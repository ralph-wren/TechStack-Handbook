...
sequenceDiagram
  ...
  participant SlotProvider
  participant ResourceManager
  participant TaskManager
  participant Task

  Scheduler->>SlotProvider: requestSlot(requirements)
  SlotProvider->>ResourceManager: requestSlot()
  ResourceManager->>TaskManager: requestSlot()
  TaskManager->>ResourceManager: offerSlot()
  ResourceManager->>SlotProvider: slotOffered()
  SlotProvider->>Scheduler: slotAllocated()
  
  Scheduler->>TaskManager: deployTask(descriptor)
  TaskManager->>Task: create Task instance
  Task->>TaskManager: task created
  TaskManager->>Scheduler: deployment acknowledged
  
  Task->>TaskManager: task running
  TaskManager->>Scheduler: updateTaskState(RUNNING)
```
